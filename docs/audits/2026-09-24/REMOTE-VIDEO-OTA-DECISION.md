...
**Обновлено 25 сентября 2026 · pilot `sphere-pilot-20260911` · P0 для Fleet32 · статус: причина потери удалённого изображения не локализована, массовый rollout — NO-GO. Backend queue fix уже на pilot; 1.2.18 установлена на одном локальном rooted canary; удалённая OTA и terminal receipt не подтверждены.**
...
**Дополнение 24 сентября вечером UTC:** исправленный backend image
`b2562ca04f5f` теперь healthy на pilot; read-only тест внутри контейнера
подтвердил, что 3-byte IDR распознаётся и сохраняется очередью. Повтор через
временный LocalTunnel viewer получил от `PH008` только SPS/PPS, без IDR/P.
Попытка перевести только `PH008` на альтернативный primary с Cloudflare fallback
завершилась `504` без receipt; команда чтения логов `PH007` тоже завершилась
`504`. Поэтому Android egress фактически не подтверждён на новом маршруте, а
Cloudflare не доказан как причина. Сводка и точные пределы эксперимента — в
[AUD-164](REMOTE-INGRESS-AB.md).
...
### Runtime-срез до обновления backend, 24 сентября
...
**Статус:** исходный дефект исправлен в PR #19 (`7a9bfea`) и затем развёрнут на
pilot в `b2562ca04f5f`. **Доказательство:** до fix red/green queue regression
дала 5 failed/26 passed; после fix прошли 83 stream-теста. В 17:08 UTC
проверка внутри тогдашнего `ff87b56` повторила отказ: трёхбайтовый IDR стал
некритичным и был удалён очередью; четырёхбайтовый контроль сохранился.
Read-only probe в текущем `b2562ca04f5f` подтвердил обратное: трёхбайтовый IDR
получает `nal_type=5`, `is_critical=true`, а P-frame `nal_type=1`,
`is_critical=false`. Сохранённый PH006/PH008 capture не содержит сырых IDR bytes,
поэтому связь именно этого пропуска картинки с трёхбайтовым prefix не доказана.
**Файлы:**
...
пока неизвестной точкой потери. RV-13 — исправлен в source и развёрнут на pilot;
связь исходного queue-дефекта с конкретным удалённым кадром не подтверждена, а
текущий remote capture по-прежнему не содержит IDR/P.
...
- Текущая правка prompt OTA scheduling проверена полной dev и enterprise suite:
  ...
  приёмкой.
- OTA storage source-fix проверен через 9 локальных тестов после красного
  воспроизведения: effective local-pilot/production Compose теперь включает
  persistent volume; resolver не использует `/tmp`. Ни контейнеры, ни OTA runtime
  не перезапускались и старый каталог не мигрировали, поэтому сохранность APK на
  живом стеке пока не измерена.
