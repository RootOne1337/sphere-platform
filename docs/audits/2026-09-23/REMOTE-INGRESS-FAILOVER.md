...
## Последующий Android source follow-up

Текущий report не завершает диагностику удалённого маршрута. В отдельной проверке
**AUD-161** детерминированно воспроизведён Android race: первая выдача ImageReader
могла прийти до того, как `StreamingManagerImpl` отмечал захват активным. Ранний
callback возвращался до `acquireLatestImage()`, оставляя неподвижный первый экран
без picture NAL. Source fix и обе полные Android test-матрицы прошли; это ещё не
доказывает, что именно этот путь вызвал отказ удалённого экземпляра. Нужна установка
кандидата на один canary и одновременное подтверждение encoder output, backend frame
receipt и browser decode. См. [AUD-161](ANDROID-INITIAL-FRAME-RACE.md); Cloudflare
остаётся возможным фактором только если APK подтверждает принятые к отправке кадры,
которые сервер не получает.
