...
| 2 | Dependencies/CI | Combined SQLite/PostgreSQL tests исправлены; CI включает real-service regressions, конечный timeout и artifacts; Host→audit/log/metrics path poisoning воспроизведён и исправлен в middleware, dependency advisories открыты | Совместимое обновление с проверкой resolver/runtime, dependency-aware mypy; отдельный подготовленный load job; GitHub checks на последнем head без снижения gates |
