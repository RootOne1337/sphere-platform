...
| 2 | Dependencies/CI | Combined SQLite/PostgreSQL tests исправлены; CI включает real-service regressions, конечный timeout и artifacts; Host→audit/log/metrics path исправлен; совместимое Python обновление: 1047 tests, pip check и joint pip-audit без известных уязвимостей локально | Проверить GitHub CI нового head; frontend/Android/container advisories, hash lock/SBOM, dependency-aware mypy; отдельный подготовленный load job |
