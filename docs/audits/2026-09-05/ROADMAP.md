...
| 2 | APK runtime и производительность | 318 JVM tests; typeText больше не пишет raw/encoded ввод в логи; сервер восстанавливает evicted presence по pong; реальный APK↔API и нагрузка 10–64 не завершены | FGS/boot/timeout, emulator/physical permissions, process death, codec backpressure/recovery, multi-worker session fencing, PubSub reconnect, CPU/RAM/FPS/battery |
