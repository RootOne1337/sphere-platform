...
| --- | --- | --- | --- |
| 1 | Task lifecycle | SQL assignment/receipt recovery, Android journal, TaskService producer/cancel serialization, batch counters и UTC watchdog исправлены | Task-specific durable cancellation и stop ACK, Redis/commit failure при cancel, batch/scheduler cancellation, stale RUNNING reconciliation, pipeline/scheduler producers, post-commit webhook/events, повторные/запоздалые команды |
