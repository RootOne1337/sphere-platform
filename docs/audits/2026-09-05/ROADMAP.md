...
| --- | --- | --- | --- |
| 1 | Task lifecycle | SQL assignment/receipt recovery, Android journal, TaskService producer/cancel serialization, distinct control receipts, APK target matching/checkpoints в loop/retry/final outcome, batch cancel/result serialization, batch counters и UTC watchdog исправлены | Durable cancellation/stop ACK, ordering controls той же задачи, Redis/commit failure при cancel, scheduler cancellation, wave producer cancellation/status, stale RUNNING reconciliation, pipeline/scheduler producers, post-commit webhook/events |
...
| 2 | Dependencies/CI | Совместимое Python обновление: 1062 tests, pip check и joint pip-audit без известных уязвимостей; на 71b2d2c все backend jobs и Android CI успешны; Host→audit/log/metrics path исправлен | Frontend/Android/container advisories, hash lock/SBOM, dependency-aware mypy, actions runtime/version pins; отдельный подготовленный load job |
