...
| 2 | Dependencies/CI | Совместимое Python обновление: 1109 tests, pip check и joint pip-audit без известных уязвимостей; на 788a625 все backend jobs и Android CI успешны; Host→audit/log/metrics path исправлен | Frontend/Android/container advisories, hash lock/SBOM, dependency-aware mypy, actions runtime/version pins; отдельный подготовленный load job |
