...
| 1 | VPN | Device ownership/PSK/revoke errors и health poll auth/unknown state/SQL commit/overlapping observations исправлены; дубли IP/reinit остаются | [План durable lease](VPN-LEASE-DESIGN.md): глобальная SQL reservation до router effects, generation fencing, provider reconciliation, безопасная миграция дублей; AWG/маршруты и реальный command publisher |
