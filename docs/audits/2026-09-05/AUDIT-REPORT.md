...
**Новая контрольная точка, 26 сентября 2026:** две локальные APK 10228
автономно поднялись после полного OFF/ON LDPlayer; второй экземпляр перед этим
установил 10228 через собственную OTA. Для удалённого PH025 доказан отказ OTA
из-за сравнения только активного host, а повторная адресная доставка и запрос
журнала не получили квитанцию при статусе `online`. Source fix с регрессионным
тестом поднят до 10229; массовый rollout остаётся **NO-GO**.
[Метод, доказательства, fix и residual risk](../2026-09-26/ANDROID-COLD-BOOT-AND-OTA-CANARY.md).
Предыдущие датированные разделы ниже — исторические срезы, а не текущая
инвентаризация парка.

Статус на 24 сентября 2026: **аудит продолжается; production readiness не подтверждена**.
Последнее документированное состояние live pilot приведено отдельно ниже; следующие
source fixes в PR не считаются установленными до независимой runtime-приёмки.

**AUD-143 · P1 / High operational · OTA release catalog, source fix:** два
независимых процесса публиковали разные версии; оба получали HTTP 201, но одна
запись терялась. Baseline воспроизведён на production handler в изолированном
каталоге. Межпроцессная блокировка теперь охватывает create/delete, запись выполняется
через synced temporary file и atomic replace, повреждённый каталог отвечает 503,
а не выглядит пустым. После fix прошли 32 теста OTA API/regression suite и Ruff.
[Root cause, воспроизведение, файлы и границы rollout](../2026-09-24/OTA-CATALOG-CONCURRENCY.md).
Fix находится в исходниках PR #19 и пока не проверен на live pilot. Блокировка
рассчитана только на процессы одного узла и совместимую локальную файловую систему;
для нескольких backend hosts потребуется транзакционный каталог и object storage.
...
ошибка SQL conflict check и повторный SKIP просроченного интервала. Семь failures
до fix → 86 related tests passed: две организации/workers, потеря commit ACK,
...
БД/Redis, APK/video, UI, VPN, PC-agent, deployment и observability: 24 приоритетных
пункта с раздельными уровнями доказательств. Семь новых диагностических сценариев
подтверждают открытые дефекты SQL orchestration/cancel/OTA; активный decoder проверен
со stalled codec double. Это анализ и воспроизведения, не заявление о внесённых
fixes или capacity 32. Старые residual risks ниже уточнены, но не скрыты.

**20 сентября · AUD-126: несколько просмотров одного устройства.** На двух APK
подтверждено: новое окно вытесняет прежние, хотя они остаются открытыми. Исправлена
доставка каждому viewer и освобождение подписки только после последнего.
3 failing regressions → 112 связанных tests passed. Backend `85fb1ea` установлен;
шесть native-этапов с тремя/двумя/одним viewer прошли, оба APK сохранили PID и
crash buffers. [Причина, воспроизведение и границы](STREAM-MULTI-VIEWER.md).

**AUD-127:** ночной harness раньше ошибочно принимал начальный кадр extra-viewer
за успешный overlap. Теперь требует новое видео каждому зрителю при открытии и
после закрытия дополнительного окна: 3 failing regressions → 22 harness tests,
плюс шесть настоящих проверок helper. [Критерий и ограничения](SOAK-VIEWER-MOTION.md).
...
[Воспроизведение и ограничения](SCRIPT-CREATE-RESPONSE.md).
...
совпадают, новых crash records после OTA нет. 560 full JVM tests.
[Инцидент, evidence и границы](ANDROID-CAPTURE-LIFECYCLE.md).

**AUD-113:** здоровый статичный экран приводил к повторному start каждые 5 s
из-за Redis request timeout в `listen()`. Настоящий baseline падает; после
bounded message polling 212 связанных tests pass. Backend `fa099aa` установлен
только на новом стенде. [Root cause и приёмка](STREAM-IDLE-RECOVERY.md).

**AUD-109–111:** bounded video queues, restricted-role viewer login и cross-worker
frame/control/lifecycle исправлены. Оба реальных экрана декодируются в браузере;
первичная стабильная приёмка на старой 1.2.4 была отклонена из-за AUD-112.
GET stream status между workers, длительный soak и fleet capacity остаются открыты.
[Дефекты, evidence, residual risk](STREAM-VIDEO-ROUTING.md).
...
[Контракт и открытая native-приёмка](ANDROID-OTA-DELIVERY.md).

**AUD-104:** OTA worker перестал считать HTTP 401/429/503 успешной проверкой.
9 before failures → 14 worker regressions и 547 full JVM tests pass.
Root installer на втором Android работает без диалога; серверная публикация
и полная native-доставка проверяются отдельно. [Evidence](ANDROID-OTA-DELIVERY.md).
...
[Причина наблюдаемого отказа, инструмент ремонта, 15 tests и residual risk](LDPLAYER-NAT-INCIDENT.md).
Follow-up: [автоматический host watchdog](LDPLAYER-AUTOMATIC-RECOVERY.md) установлен
для indices 0/1; 56 tests. Настоящий fault → штатный scheduled repair → команда APK
через 108.12 s, затем 12/12 с сохранением PID. Статус пока локальный, reboot не принят.
...
потерю результата при сбое транспорта и несогласованность PostgreSQL с Redis.
