...
**Обновлено 24 сентября 2026 · решение: NO-GO для длительного прогона 32 устройств с задачами и отказами.**
...
**Новый live-срез 24 сентября:** [AUD-164](../2026-09-24/REMOTE-INGRESS-AB.md)
заменяет численные показатели следующего датированного абзаца: в API теперь
10 записей, а три новых remote ID `PH007`–`009` имели 19–21 reconnect за
15 минут, без подтверждённого heartbeat. Независимый viewer получил от
локального Android IDR/P, но от удалённого PH008 только SPS/PPS, как и локальный
viewer. Android egress оставался Cloudflare, поэтому его вина ещё не доказана.
...
показали online без переустановки APK. Это частичная приёмка clone identity, не
подтверждение полного флота из 22 устройств. Локальный `auto-ph-000` передал IDR/P
через публичный Quick Tunnel; удалённый PH006 — только SPS/PPS и ни одного IDR/P.
**F32-28 продвинут для семи записей, но остаётся OPEN до подтверждения всех VM;
remote video, F32-29/31 acceptance, F32-36/37 и Fleet32 fault/soak остаются OPEN.**

Более ранний срез [AUD-147, 16:12](../2026-09-23/REMOTE-FLEET-LIVE-AUDIT.md)
показал три новых offline-карточки и токен/WS churn на старом backend. Он был
устранён backend-first rollout. Quick Tunnel остаётся временным тестовым каналом без
SLA; сравнение локального и удалённого устройства не доказало, что именно tunnel
теряет remote IDR. Нужны локальные capture/encoder/send counters и независимый WAN
route test. Текущий результат не разрешает длительный прогон на 32 устройствах.
...
прошло весь набор frontend-тестов и проверку TypeScript. Это устраняет пробел
восстановления браузера, но не локализует ранее наблюдавшиеся на удалённом просмотре
SPS/PPS без IDR. Новый frontend ещё не установлен; удалённая приёмка видео одного
устройства остаётся OPEN.

**23 сентября — P0 F32-31:** Android также терял одноразовый `viewer_connected`, если
команда приходила до создания encoder. Добавлен отложенный sync-frame request и
диагностика отказа MediaCodec; viewer-to-agent backend path покрыт регрессией.
108 связанных JVM-тестов прошли для каждого APK flavor, backend suite — 31 passed.
Это исходный код, не удалённый rollout: обе точки ещё требуют проверки кадров на
одном реальном WAN-устройстве. [Доказательства](ANDROID-KEYFRAME-STARTUP.md).
...
**Текущая source-проверка, 23 сентября:** APK-кандидат собран из полного исходного
head `6788c90704319b5cf17ea0d92c7803220ba1c9a0`: `1.2.11-dev / 10211`, файл
`.local-pilot/apk/SphereAgent-pilot-candidate-1.2.11-dev-6788c90.apk`, SHA-256
`18935e44b43b2f731176677a2acf8d306821792eee0477901a4f2606a76e73b7`. Встроенный
`GIT_SHA`, пакет, версия и v2 подпись проверены; 610 Android tests прошли в каждом
flavor, с одним существующим skip в каждом. Полный CI кода на `6788c90` прошёл;
последующий docs-only head `cbb5e72` проверяется отдельно. Candidate не установлен,
не опубликован; OTA alias/catalog остаются на принятой 1.2.9. Backend v2 rollout,
20 отдельных удалённых VM identity и реальный IDR → browser decode не подтверждены.
Поэтому **Fleet32 остаётся NO-GO**, несмотря на зелёные source checks.
