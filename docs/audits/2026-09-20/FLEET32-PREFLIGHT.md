...
**22 сентября — новый P0 F32-28:** [копии APK используют общий device_id](CLONE-IDENTITY.md).
20 удалённых + 2 локальных VM представлены тремя карточками; подтверждены
вытеснения соединений и отсутствие IDR. Приоритет — миграция клонов и повторная
native проверка стрима, затем прежние F32-07/08. Source fix не означает rollout.
