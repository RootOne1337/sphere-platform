...
- **Repeat probes:** PR head `6868f92`, CI run `35795606456`, passed with `passed=true`,
  network `none`, no published ports, container removed, limit/swap each 2,147,483,648
  bytes, AOF/BGSAVE/restart passed, 6,531 keys and marker survived, `OOMKilled=false`,
  restart count zero. Its cgroup peak was **1,603,907,584 bytes** (~1.49 GiB). PR head
  `7af4428`, CI run `35797003733`, passed the same checks; 6,532 keys and marker
  survived, `OOMKilled=false`, restart count zero, and peak was **1,974,636,544 bytes**
  (~1.84 GiB). Across three post-fix runs the observed peak range is 1.49–2.00 GiB;
  the maximum reached the limit. The variation means no run establishes dependable
  reserve for streams. Backend JUnit: 1,997 tests, 0 failures/errors, 15 skipped. APK,
  frontend, security, lint, RLS, image bootstrap and Alembic gates passed on all heads.
...
and Docker host aggregate memory. All three isolated 2048 MiB probes passed; peaks
were 1.49, 1.84 and 2.00 GiB. Use the maximum observed peak and host-level
measurements; do not treat lower reruns as guaranteed margin or stream capacity.
