...
официальным; изолированные попытки с новым и прежним ключом **этого pilot** получили
...
получила receipt за 18 секунд (`504`); запрос логов для `PH007` также не получил
ответа за 15 секунд. Поэтому фактический egress Android не переключён/не
подтверждён и сравнивать провайдеры пока нельзя. Временный LocalTunnel после
остановки процесса возвращал `503`; его нельзя считать резервом. Действующий
Cloudflare connector и signed config не менялись.
