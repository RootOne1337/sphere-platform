...
**25 сентября, 00:00 Asia/Yekaterinburg (24 сентября, 19:00 UTC):**
[AUD-164 — независимый viewer ingress и три remote VM](../audits/2026-09-24/REMOTE-INGRESS-AB.md) ·
[AUD-163 — удалённое видео и решение по OTA](../audits/2026-09-24/REMOTE-VIDEO-OTA-DECISION.md).
Pilot backend `b2562ca04f5f`, frontend `48c9480`, Cloudflare connector и
public-gateway healthy. Signed public endpoint ответил health `200`; WSS прошёл
upgrade и закрыл тест с ожидаемым кодом auth `4001`. В работающем backend
синтетический 3-byte IDR классифицирован как critical, а P-frame — как обычный;
фикс очереди развёрнут, но это не создаёт IDR на удалённом Android.

Короткий просмотр `PH008` через временный альтернативный ingress получил только
2 валидных пакета, 61 байт (SPS=1, PPS=1, IDR/P=0). Попытка адресно переключить
Android на него получила `504` без receipt; чтение логов `PH007` тоже получило
`504`. Поэтому Android egress A/B не подтверждён и гипотеза Cloudflare остаётся
открытой. Временный ingress после остановки вернул `503` и резервом не является.
Cloudflare, общий signed manifest, APK и OTA каталог в этом эксперименте не
менялись.

Последняя API-выборка показала 10 записей, а не заявленный удалённый парк из 20
клонов. `PH007`–`009` отображаются online, но в том же срезе heartbeat у `PH007`
отсутствовал, а у `PH008`/`PH009` был старше 10 секунд; `online` не является
приёмкой. Фактические версии APK на удалённых VM не получены. Глобальная
OTA-публикация и Fleet32 остаются **NO-GO** до подтверждённого Android egress,
IDR/P в backend и декодированного кадра после reconnect. Записи ниже — датированные
исторические срезы, а не установленное сегодня состояние.
