...
**Срез: 24 сентября 2026 · аудит продолжается · Fleet32 остаётся NO-GO.**

## Текущее состояние после обновления

**24 сентября:** [AUD-164 — независимый viewer ingress и три remote VM](../audits/2026-09-24/REMOTE-INGRESS-AB.md) ·
...
Quick Tunnel; локальный `emulator-5554` обновлён адресным OTA до 1.2.18/10218,
`emulator-5556` остаётся на 1.2.9/10209. Последняя API-выборка
показала 10 записей, включая три новые удалённые `PH007`–`009`. Все три
переподключались 19–21 раз за 15 минут и не имели подтверждённого heartbeat;
online не является приёмкой 20 удалённых клонов. Обычный backend OTA catalog `android/dev` предлагает максимум
...
установку. Локальная установка 1.2.18/10218 подтверждена только на одном
эмуляторе; terminal backend receipt отсутствует. Candidate APK 1.2.19/10219
с быстрым OTA check собран из `6c000ea`, signer и signed-discovery config сверены,
полные suites прошли; кандидат не установлен или опубликован. Точные hash и
путь указаны в [Local Pilot](LOCAL-PILOT.md#apk-именно-для-нового-стенда). Обычный
OTA канал не изменялся;
1.2.16/10216 и 1.2.17/10217 заменены до rollout исправлениями очереди OTA receipts.
PH006 и новый PH008 получали только SPS/PPS без IDR/P; локальный
контроль через тот же публичный tunnel и независимый viewer ingress передал IDR.
Причина удалённого отказа
...
Записи ниже — датированные исторические срезы, а не установленное сегодня состояние.
