...
потери одноразового запроса IDR. В код добавлены повторы с растущей паузой и явное
состояние ожидания изображения; тесты frontend и проверка TypeScript проходят.
Удалённые SPS/PPS без IDR всё ещё надо локализовать по цепочке захват экрана →
энкодер → APK → backend/Redis → декодер браузера; изменение пока не установлено на
pilot. См. [доказательство и остаточный риск](../audits/2026-09-20/STREAM-FIRST-FRAME.md).
