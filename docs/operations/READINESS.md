...
`PackageInstaller`. Из него собран локальный pilot candidate:
`.local-pilot/apk/SphereAgent-pilot-candidate-1.2.28-dev-add00b3.apk`, SHA-256
`15abeee280bf743e67e544ed94b36575590557c3353d73e911d1279ab2946ce9`, размер
8,430,789 байт, package `com.sphereplatform.agent.pilot.debug`. Подпись v2
проверена и совпадает с локальным pilot signer. Артефакт содержит dev pilot
enrollment-конфигурацию и не является production release. Он **не установлен**
на canary, не загружен в GitHub Releases и **не опубликован в OTA**.

Последние локальные версии, измеренные в 15:58 UTC, были
`emulator-5556 = 1.2.27-dev` и `emulator-5554 = 1.2.23-dev`; после этого новых
запросов к эмуляторам не делалось. Локальный promoted-artifact manifest,
проверенный в 17:07 UTC, всё ещё указывает на `1.2.9-dev / 10209` с SHA-256
`b4bf3f319f92e1f3ac24b7d68dffdc6f6d93a113ad59b10b132ea8f60e641078`; он не
доказывает состояние удалённого backend-каталога. Версии удалённых устройств
в этом проходе не измерялись. Значит, сейчас нельзя утверждать, что исправление
дошло даже до локального canary или удалённых устройств.

OTA worker запускает проверку при старте и аутентифицированном reconnect, затем
периодически раз в 6 часов. Этот факт сам по себе не означает доступность новой
версии: backend OTA-каталог должен содержать именно продвигаемый артефакт.
Текущий локальный candidate подписан debug/pilot identity и содержит dev
enrollment-конфигурацию, поэтому он не готов к публичному или production OTA.
Перед canary нужен допустимый для выбранного окружения подписанный артефакт и
явная публикация; после неё для каждого device ID надо сверить receipts
проверки, скачивания, SHA-256, установки и post-install heartbeat. Массовая
раскатка остаётся **NO-GO**.
...
## Исторический live-срез — 26 сентября, 05:59 Asia/Yekaterinburg

Pilot `sphere-pilot-20260911` всё ещё использует backend `2168c33` и frontend
`fc65b55`; backend healthy, в рамках этой корреляции контейнеры не обновлялись
и не перезапускались. API показывает 15 записей: четыре online (`010/011/022/023`),
семь connecting (`012/013/014/016/017/019/020`) и четыре offline/без live-key
(`015/018/021/024`). Локальными подтверждены `010/011`; удалёнными — `022/023`.

У `022/023` пользователь видит картинку и клики, оба отвечают на heartbeat и
PING, а backend runtime counters фиксируют ingress и viewer-send кадры по
текущему Cloudflare ingress. Это реальный удалённый успех для двух устройств,
не подтверждение всего парка, FPS, длительного soak или резервного failover.
Оба устройства сейчас отчитываются как `1.2.22-dev / 10222`; APK-кандидат
`1.2.23 / 10223` установлен только на локальный canary `5554`. У устройств в
`connecting` версии не получены, так что утверждать, что на всех стоит одна
версия, нельзя.

`Connecting` означает, что backend принял аутентифицированный WebSocket, но не
получил application-level heartbeat/pong. Для `019` тестовый PING был опубликован
одному subscriber, но receipt не пришёл за 8 секунд; группа `012–020` часто
переподключается с close `1005`. Точная причина на Android пока неизвестна.
Серверные данные не подтверждают Cloudflare как единственную причину: успешные
`022/023` тоже вошли через cloudflared. Published discovery v25 включает
независимый LocalTunnel fallback, но ни одного Android WebSocket через него
пока не наблюдали.

Отдельная ошибка синхронизации transient `connecting` в PostgreSQL enum
воспроизведена; локально исправлена нормализацией persisted статуса до `offline`.
Целевой regression до фикса падал, после фикса 6 тестов и Ruff прошли. В pilot
исправление ещё не раскатано; оно не объясняет отсутствие pong. APK resource
profile (CPU/RAM/энергопотребление) на десятках эмуляторов в этом проходе не
измерялся. Массовая OTA и тест на 32 устройствах остаются **NO-GO**.

Подробная корреляция и границы доказательств: [удалённый управляющий канал](../audits/2026-09-26/REMOTE-CONTROL-PATH-DIAGNOSIS.md) ·
[APK/presence и release gate](../audits/2026-09-26/ANDROID-PRESENCE-AND-APK-RELEASE.md).

## Контрольная точка rollout — 26 сентября, 02:05 Asia/Yekaterinburg (историческая)
...
**Read-only live-срез 23 сентября, 16:12:** [AUD-147 — три новые offline-карточки и
отсутствие удалённого видеокадра](../audits/2026-09-23/REMOTE-FLEET-LIVE-AUDIT.md).
В PostgreSQL 6 записей: 3 прежних online и 3 созданных сегодня offline; Redis
подтверждает только 3 online. За 30 минут общая удалённая карточка получила
22 вытеснения соединения и 565 отказов `Token expired`. Новый backend v2, новый
frontend и APK 1.2.11 ещё не применены к pilot; опубликованная OTA — 1.2.9.
Это состояние на момент той проверки. [Ревью кода 22–23 сентября](../audits/2026-09-23/REMOTE-FLEET-LIVE-AUDIT.md#ревью-качества-кода-2223-сентября)
подтвердило локальные исправления и выявило F32-37: новый viewer при живых ping,
но без кадра, может бесконечно показывать ожидание. Это не установившаяся причина
нынешней потери удалённого IDR. Текущий Quick Tunnel также проверен как
[высокоприоритетная гипотеза WAN-отказа](../audits/2026-09-23/REMOTE-FLEET-LIVE-AUDIT.md):
Cloudflare поддерживает WS, но этот тестовый маршрут без SLA, а передача больших
данных из России требует отдельной проверки. Fleet32 остаётся **NO-GO**.
...
продолжают конкурировать за старый общий ID; ещё три новые записи пока offline.
Ниже — предыдущая приёмка двух
...
standalone-сервер и четыре маршрута прошли HTTP-проверку на localhost. Linux CI на
`2cb4a9d` проверил сборку и наличие корневого entrypoint; отдельный frontend Docker-image
