...
**Продвижение:** [новый локальный Compose](LOCAL-PILOT.md) поднят отдельно от старой
установки; migration/admin/enrollment, browser login, dashboard/devices и session
refresh после reload проверены. Подготовлен отдельный pilot APK. Дальше — установка
на эмулятор и настоящее задание; рубеж A пока полностью не закрыт.
...
12 сентября выполнен Compose/browser smoke. Установленный APK ещё не проверен;
in-process HTTP и JVM tests не заменяют этот этап.
