...
| **Удалить** | Диалог подтверждения | Удаляет запись из каталога Sphere. Не удаляет APK или приложение на Android; работающий агент может зарегистрировать запись повторно. |
...
| **Удалить** | `Trash2` | `DELETE /api/v1/devices/bulk`; необратимое удаление записей каталога в пределах текущей организации, максимум 500 ID. Ошибка не очищает выделение; частичный ответ оставляет доступные записи выделенными. APK не затрагивается. |
