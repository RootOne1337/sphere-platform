...
> **Актуальность, 25 сентября 2026:** этот справочник описывает поведение исходного кода
> в ветке аудита. Он не подтверждает, что frontend/backend уже развернуты на рабочем сервере.
> Для Fleet Matrix приложен отдельный [аудит интерфейса и массовых операций](audits/2026-09-25/FLEET-MATRIX-BULK-ACTIONS.md);
> состояние потоков и pilot проверяются отдельно: [stream acceptance](audits/2026-09-20/DECODER-RECOVERY.md),
> [состояние pilot](operations/LOCAL-PILOT.md), [fleet preflight](audits/2026-09-20/FLEET32-PREFLIGHT.md).
...
Таблица и плиточный просмотр зарегистрированных Android-устройств. Экран запрашивает до 5 000 записей на страницу и использует виртуальный список TanStack; массовые API-операции принимают не более 500 устройств за запрос.

### Шапка страницы

| Элемент | Описание |
|---------|----------|
| **Сводка** | Количество записей и текущие статусы из API: online, offline, error/unknown. Это не сетевой ping и не доказательство доступности видеопотока. |
| **Поиск** | Запрос к API с задержкой 300 мс; группа и локация дополнительно фильтруются на уже загруженном наборе. |
| **Таблица / сетка** | Переключает виртуальную таблицу и просмотр потоков; выбранные в таблице устройства ограничивают набор в сетке. |
| **Обновить список** | Повторяет запрос. При ошибке загрузки экран показывает сообщение и не подменяет сбой пустым каталогом. |

### Таблица FleetMatrix

Строки виртуализированы, высота строки — 48 px. Доступны сортировка по колонкам и настройка видимых полей.

| Колонка | Тип | Описание |
|---------|-----|----------|
| Выбор | Checkbox | Выбирает только устройство из текущего видимого результата. Смена поиска/фильтра очищает скрытое выделение. |
| Идентификатор | Имя + метаданные | Имя открывает Inspector; ниже отображаются модель, Android и версия агента из записи устройства. |
| Статус | Badge | Показывает API-статус, включая connecting, busy, maintenance, error и unknown. |
| Питание | Значение | Последний известный заряд батареи; пустое значение показывается как `—`. |
| Доступ | Флаги | Отображает поля `adb_connected` и `vpn_assigned`; это не проверка текущего туннеля. |
| Игровой сервер | Текст | Значение `server_name`, если оно назначено. |
| Метки | Badge[] | До трёх меток и счётчик остальных. |
| Последняя связь | Время | Значение `last_seen` из API; отдельного измерения RTT эта таблица не делает. |
| Действия | Меню | Переименование, назначение группы/локации/сервера и удаление записи каталога. |

### Контекстное меню устройства (⋯)

| Действие | Диалог | Описание |
|----------|--------|----------|
| **Переименовать** | Dialog | `PUT /api/v1/devices/{id}`; закрывается после ответа сервера, при отказе показывает причину. |
| **Группа** | Dialog + Select | `POST /api/v1/groups/{group_id}/devices/move`; API возвращает фактическое число обработанных записей. |
| **Локация** | Dialog + Select | `POST /api/v1/locations/{location_id}/devices`; ответ считает новые привязки, повторное назначение уже связанной записи идемпотентно. |
| **Игровой сервер** | Dialog + Select | Обновляет `server_name` через `PUT /api/v1/devices/{id}`. Список серверов приходит из pipeline settings. |
| **Удалить** | Диалог подтверждения | Убирает запись из активного каталога Sphere и отзывает refresh-доступ. История задач и событий сохраняется; APK и данные Android не удаляются. |

### Bulk Actions (массовые операции)

Появляются при выделении устройств в таблице. Максимум — 500 ID за запрос; при большем выделении операции блокируются с объяснением. После смены поиска/фильтра действия применяются только к оставшимся видимыми выбранным строкам.

| Кнопка | Иконка | Описание |
|--------|--------|----------|
| **Перезапустить** | `RefreshCcw` | `POST /api/v1/devices/bulk/action`. Backend отправляет каждому online-агенту `REBOOT` через live command channel и принимает `received`, `running` или `completed`. Offline, timeout, отсутствующий транспорт и отказ агента возвращаются как ошибки конкретных устройств. ACK означает приём команды, а не доказательство завершившейся перезагрузки. |
| **Группа** | `FolderOpen` | `POST /api/v1/groups/{group_id}/devices/move`; замена текущей группы. При частичном результате выделение сохраняется. |
| **Локация** | `MapPin` | `POST /api/v1/locations/{location_id}/devices`; аддитивная привязка. Toast показывает число новых связей, а не выдуманное число успешно добавленных устройств. |
| **Переименовать** | `Pencil` | Появляется при выборе одного устройства. |
| **Отозвать VPN** | `ShieldOff` | `POST /api/v1/vpn/revoke/bulk`; отдельный результат на устройство, частичный сбой сохраняет в выделении только неуспешные записи. Требует `vpn:mass_operation`. |
| **Удалить** | `Trash2` | `DELETE /api/v1/devices/bulk`; переводит до 500 owned-записей в неактивное состояние и скрывает их из стандартного списка. История и внешние ключи сохраняются, refresh-токены отзываются; Android APK не затрагивается. Ошибка сохраняет выделение. |

```
Пример: выбрано 15 устройств → POST /api/v1/devices/bulk/action
{ "device_ids": ["..."], "action": "reboot" } → per-device results
```

Все destructive/operational действия показывают pending/error/success состояние. Backend-ответы групп и локаций не приравниваются к числу выделенных устройств: UI показывает фактический счётчик API. Fleet Matrix не подтверждает, что изменения этой ветки уже установлены на вашем сервере.
...
| Элемент | Описание |
|---------|----------|
| **Grid Layout Selector** | Число карточек: 1, 2, 4, 6, 9, 12, 16, 25, 32 или 64; экран показывает только текущую страницу устройств. |
| **Device Selector** | Поиск, фильтры по группе и локации, сортировка и пагинация списка устройств. |
| **Start/Stop** | Каждая карточка включает свой viewer; «Стоп все» закрывает все открытые viewers. |

В актуальном `MultiStreamGrid` кнопка **START** открывает viewer только для выбранных
устройств; если выбор пустой — для отфильтрованного списка. Число WebSocket viewers
ограничено размером сетки, а **Остановить потоки** размонтирует их и прекращает подписки.
Режим FIT (`contain`/`cover`) применяется к canvas с декодированным кадром. В Fleet Matrix
сетка подчиняется этому же лимиту. Метка online означает только статус из API: она не
подтверждает кадры, decode или живой viewer. Offline-плитка не выдумывает TCP-ошибку;
возраст heartbeat показывается по фактическому timestamp. См. регрессии и остаточные
ограничения в [аудите Fleet Matrix](audits/2026-09-25/FLEET-MATRIX-BULK-ACTIONS.md).
...
- Заголовок — device ID
- WebSocket: `ws(s)://host/ws/stream/{deviceId}`; схема зависит от протокола страницы
- Replay SPS/PPS передаёт параметры кодека, но картинка появится только после IDR и успешного decode
