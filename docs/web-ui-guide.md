...
- [21. Game Accounts — Игровые аккаунты](#21-game-accounts--игровые-аккаунты)
- [22. Device Events — События устройств](#22-device-events--события-устройств)
- [23. Account Sessions — Сессии аккаунтов](#23-account-sessions--сессии-аккаунтов)
- [24. Event Triggers — Триггеры событий](#24-event-triggers--триггеры-событий)
- [25. Pipeline Settings — Настройки пайплайнов](#25-pipeline-settings--настройки-пайплайнов)
...
Срез реализации и приёмки: **20 сентября 2026, `03b161e`**.

| Элемент | Контракт |
| --- | --- |
| История | Серверная пагинация по 25 задач; счётчик охватывает весь выбранный набор |
| Поиск | ID задачи, имя сценария или устройства; регистр не важен, `%` и `_` буквальные |
| Фильтр | QUEUED, ASSIGNED, RUNNING, COMPLETED, FAILED, TIMEOUT, CANCELLED |
| Сортировка | Статус, сценарий, приоритет или время создания; смена фильтра/сортировки возвращает на первую страницу |
| Success Rate | completed / (completed + failed + timeout); отменённые и незавершённые исключены |
| Активные задачи | Независимый запрос queued/assigned/running; до 10 с обозначением показано/всего |
| Активные pipeline | Настоящие run ID, статус, устройство и текущий шаг; включает waiting/paused; до 10 |
| Новый сценарий | Открывает `/scripts/builder`; существующий DAG-редактор |
| Запустить на всех | Выбор сценария и параметров волн для online-устройств организации |

Показатели загрузки и ошибки API обозначаются как недоступные. Частота обновления —
10 секунд: короткая задача может завершиться между опросами. Нажатие на название
задачи открывает `/tasks/{id}` с сохранённым результатом, node logs и доступными
действиями. Текущая страница не показывает прогноз будущих запусков или источник
CRON: приоритет не определяет, кто запустил задачу.

Проверено в браузере: 190 задач, восемь страниц, поиск самой старой задачи,
фильтр, переход в редактор и появление/исчезновение настоящего pipeline.
[Root cause, regression и ограничения](audits/2026-09-05/TASK-HISTORY.md).
...
## 21. Game Accounts — Игровые аккаунты

**Путь:** `/accounts`

Страница управления игровыми аккаунтами. Позволяет просматривать, создавать и управлять аккаунтами разных игр.

### Функциональность

- **Таблица аккаунтов** с сортировкой по игре, серверу, статусу
- **Фильтры** по игре, статусу (`active`, `banned`, `idle`), устройству
- **Создание аккаунта** — автоматическая генерация никнейма при пустом поле
- **Привязка к устройству** — назначение аккаунта на конкретное устройство
...
GET    /api/v1/game-accounts/{id}
PUT    /api/v1/game-accounts/{id}
DELETE /api/v1/game-accounts/{id}
```

---

## 22. Device Events — События устройств

**Путь:** `/events`

Таймлайн событий жизненного цикла устройств: подключение, отключение, ошибки, выполнение команд.

### Функциональность

- **Потоковый таймлайн** — реальное время обновлений через WebSocket
- **Фильтры** по устройству, типу события, временному диапазону
- **Детали события** — полный payload в JSON-формате
- **Экспорт** — выгрузка событий для анализа
...
## 23. Account Sessions — Сессии аккаунтов

**Путь:** `/sessions`

Мониторинг активных сессий игровых аккаунтов на устройствах.

### Функциональность

- **Таблица сессий** с индикаторами статуса (`active`, `ended`, `error`)
- **Фильтры** по аккаунту, устройству, статусу
- **Завершение сессии** — принудительное закрытие активной сессии
- **Метаданные** — дополнительная информация о сессии
...
GET    /api/v1/account-sessions/{id}
PUT    /api/v1/account-sessions/{id}/end
```

---

## 24. Event Triggers — Триггеры событий

**Путь:** `/event-triggers`

Конфигурация автоматических правил реагирования на события устройств.

### Функциональность

- **Таблица триггеров** с переключателями вкл/выкл
- **Создание правила** — выбор типа события, условия и действия
- **Toggle** — быстрое включение/отключение триггера
- **Условия** — JSON-выражения для фильтрации событий
- **Действия** — запуск пайплайна, отправка webhook, уведомление
...
GET    /api/v1/event-triggers/{id}
PUT    /api/v1/event-triggers/{id}
DELETE /api/v1/event-triggers/{id}
POST   /api/v1/event-triggers/{id}/toggle
```

---

## 25. Pipeline Settings — Настройки пайплайнов

**Путь:** `/pipeline-settings`

Редактор пользовательских настроек для каждого пайплайна.

### Функциональность

- **Список настроек** с группировкой по пайплайну
- **Key-Value редактор** — создание и редактирование параметров
- **Типы значений** — строка, число, JSON-объект
- **Привязка к пайплайну** — настройки автоматически применяются при запуске

### API-эндпоинты

```http
GET    /api/v1/pipeline-settings
POST   /api/v1/pipeline-settings
DELETE /api/v1/pipeline-settings/{id}
```
