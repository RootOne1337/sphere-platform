...
**Решение от 12 сентября 2026. Статус: проект следующего этапа; реализованные
части и открытые работы перечислены отдельно.**
...
| Начальные источники | **Один**, список независимых mirrors пока не реализован |
| Доверие документу | HTTPS/проверка URL; **подписи, durable config version, anti-rollback и installation binding пока нет** |
| Новый адрес для первоначальной регистрации | AUD-90 использует baked key только для baked пары. Перенос на совершенно новый адрес через routes-only JSON пока недостаточен |
...
`installation_id` в JSON тестового gateway сейчас служит идентификатором для
acceptance probe; Android пока не проверяет это поле. Поле `config_version` также
не означает, что APK уже защищён от устаревшего зеркала.
...
Если нужен автоматически меняющийся туннель, потребуется небольшой контроллер
на серверной стороне: получить новый URL, проверить его, подписать и опубликовать
обновление на независимых config hosts. Только retry в APK эту задачу не решает.
Такой контроллер пока не реализован; ключ публикации/подписи не передаётся APK.
