...
| AUD-157 | P2 | MultiStreamGrid displayed a fixed `ERR_CONN_REFUSED` string whenever a device was offline, regardless of the actual cause, and claimed `< 5s` heartbeat freshness without measuring it. | Fixed in source in the 2026-09-25 Fleet Matrix audit; UI now shows offline guidance and the age of the API heartbeat. | Source-only until frontend rollout. This is not proof of stream health; viewer frame freshness remains a separate signal. |
...
| [AUD-162](../audits/2026-09-24/REMOTE-RECONNECT-INCIDENT.md) | P1 | The deployed APK path treats refresh `401` as transient and reuses the rejected access token. A two-hour pilot window recorded 1,929 refresh `401` responses and 1,956 Android `invalid_token` events; one anonymized device identity accounts for the invalid-token loop. | Fixed in source and covered by a red/green Android regression: durably clear the rejected credential pair, retain identity/binding/routes, then use the existing enrollment guard. Policy/ingress `403` and transient network/`5xx` do not trigger re-enrollment. | APK 1.2.14 / 10214 was the candidate at this checkpoint; the later local 1.2.15 / 10215 candidate includes it. No remote install or OTA publication is claimed. Same-window WS `101` proves only transport upgrade; no active Android encoder/queue telemetry appeared in a point-in-time metrics read, so the no-frame boundary and other session evictions still need an instrumented one-device canary. |
| [AUD-163](../audits/2026-09-24/REMOTE-VIDEO-OTA-DECISION.md) | P0 gate | Remote viewer receives SPS/PPS but no IDR/P; local agent through the same public Quick Tunnel delivers IDR/P. Backend OTA catalog still targets 1.2.9, while source candidate 1.2.15 is not deployed remotely. The normal latest catalog has no one-device cohort gate and many observed checks returned 401. | Read-only evidence separates Android, WAN/ingress, backend, browser and publication state; no tunnel, APK or backend rollout. | One remote candidate with installed-version proof, frame counters, browser decode, reconnect and, if needed, an independent ingress A/B must pass before global OTA or Fleet32. Cloudflare remains a hypothesis, not an established root cause. |
| [AUD-164](../audits/2026-09-25/FLEET-MATRIX-BULK-ACTIONS.md) | P2 | Bulk reboot wrote `cmd:reboot:{device_id}` to Redis, but repository search found no consumer; the endpoint could report success without an Android command receipt. | Fixed in source: use the live Android command channel and require a per-device receipt; offline, timeout and rejection are returned as failures. | Backend source tests only. An accepted receipt confirms command delivery/acceptance, not that the device completed reboot. Runtime rollout and device canary remain open. |
| [AUD-165](../audits/2026-09-25/FLEET-MATRIX-BULK-ACTIONS.md) | P2 | Fleet bulk delete cleared selection before the request result and offered no useful failure state; operators could not distinguish a failed hard-delete from success. | Fixed in source with a shared accessible confirmation, explicit registry-only scope, pending/error states and selection retention on failure. | Backend hard-deletes registry rows within the organization; APK and Android application stay installed, and an active agent may register again. Use with explicit operator intent. |
| [AUD-166](../audits/2026-09-25/FLEET-MATRIX-BULK-ACTIONS.md) | P2 | Fleet UI sent `vpn_revoke` to the generic bulk-action endpoint, but that action is not in its request enum; this produced a validation failure. The previous role guard also disagreed with the VPN permission matrix. | Fixed in source with a dedicated `/api/v1/vpn/revoke/bulk` contract, per-device results and `vpn:mass_operation` authorization. | Sequential VPN provider calls preserve one DB session; large batches can be slow. Use staged batches and confirm provider reconciliation before mass use. |
| [AUD-167](../audits/2026-09-25/FLEET-MATRIX-BULK-ACTIONS.md) | P2 | MultiStreamGrid's Stop action depended on an optional callback, and FIT assigned `object-fit` to a `div`; the stop control could do nothing on Fleet Matrix and FIT did not affect the decoded canvas. | Fixed in source: Stop unmounts active viewers and the selected contain/cover mode is applied to the canvas. The UI also stops claiming an unmeasured network error or fixed heartbeat age. | Component regressions verify the state/prop path and canvas styles. Browser visual and live remote-frame acceptance remain open. |
...
after encoder output stops. The 2026-09-25 Fleet Matrix source change also addresses
false-success bulk reboot, the unsupported bulk VPN action, silent destructive-action
failures, and two nonfunctional stream controls. Focused backend/frontend regressions
pass; the repository-wide frontend lint still has legacy violations outside those
changed components. These source changes do not alter the running pilot automatically.
