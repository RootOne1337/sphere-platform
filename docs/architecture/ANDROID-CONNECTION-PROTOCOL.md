...
AUD-74 использует этот ACK для выбора сохранённого маршрута: основной и резервный
адрес одной установки доступны для WS и refresh. До подтверждения активный адрес
сохраняется; рабочая сессия не прерывается discovery. Это проверка identity,
не readiness PostgreSQL/Redis. [Настройка и тесты](ANDROID-SAVED-ROUTES.md),
[HTTP discovery и локальные источники](ANDROID-DISCOVERY-RECOVERY.md).
