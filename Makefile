...
# ── Docker Compose: автоподхват .env.local (приоритет) или .env ───────────────
# Docker Compose по умолчанию читает ТОЛЬКО .env для интерполяции ${VAR}.
# make setup генерирует секреты в .env.local → нужен --env-file.
_ENV_FILE := $(firstword $(wildcard .env.local .env))
DC := docker compose $(if $(_ENV_FILE),--env-file $(_ENV_FILE))
...
full:          ## Запустить весь стек в Docker
	...
	$(DC) -f docker-compose.yml -f docker-compose.full.yml up -d
...
build:         ## Собрать production Docker образы
	$(DC) -f docker-compose.yml -f docker-compose.production.yml build

monitoring:    ## Запустить Prometheus + Grafana
	$(DC) -f infrastructure/monitoring/docker-compose.monitoring.yml up -d

logs:          ## Логи backend
	$(DC) -f docker-compose.yml -f docker-compose.full.yml logs -f backend
...
deploy-prod:   ## Полный деплой production: build → migrate → seed → up
	...
	$(DC) -f docker-compose.yml -f docker-compose.production.yml build
	$(DC) -f docker-compose.yml -f docker-compose.production.yml up -d
	@echo "Ожидание старта backend..."
	@sleep 5
	$(DC) exec backend alembic -c alembic/alembic.ini upgrade head
	AGENT_CONFIG_ENV=production $(DC) exec backend python -m scripts.seed_enrollment_key
...
ssl-renew:     ## Принудительное обновление Let's Encrypt сертификата
	$(DC) exec certbot certbot renew --webroot -w /var/www/certbot --force-renewal
...
tunnel-build:  ## Собрать Docker образ туннеля
	$(DC) -f docker-compose.tunnel.yml build
...
tunnel-logs:   ## Логи туннеля в реальном времени
	$(DC) -f docker-compose.tunnel.yml logs -f tunnel

# ── Пересборка отдельных сервисов ────────────────────────────────────────────
rebuild-backend:  ## Пересобрать только backend (при pip install)
	$(DC) -f docker-compose.yml -f docker-compose.full.yml build backend
	$(DC) -f docker-compose.yml -f docker-compose.full.yml up -d --no-deps backend
	@echo '✅ Backend пересобран и перезапущен'

rebuild-frontend: ## Пересобрать только frontend (при npm install)
	$(DC) -f docker-compose.yml -f docker-compose.full.yml build frontend
	$(DC) -f docker-compose.yml -f docker-compose.full.yml up -d --no-deps frontend
