...
async def test_interactive_request_reaches_another_worker_and_keeps_immediate_result(
    ...
    delivered = []
    replies_processed = asyncio.Event()
    ...
    async def reply(command):
        ...
        try:
            for status in ("received", "running", "completed"):
                await handle_command_result(device_id, str(world.org_a.id), {
                    "type": "command_result", "command_id": command['command_id'],
                    "status": status, "result": {"output": "isolated\n", "logcat": "isolated-log",
                        "logs": "isolated-file-log"},
                })
        finally:
            replies_processed.set()
    ...
    try:
        with patch("backend.websocket.connection_manager.get_connection_manager", return_value=api_manager), \
             ...
             patch("backend.database.engine.AsyncSessionLocal", wraps=world.sessions) as task_sessions:
            ...
            await asyncio.wait_for(replies_processed.wait(), 3)
        ...
        # HTTP auth/device authorization still uses PostgreSQL. These callbacks
        # must not open a second session looking for a nonexistent durable Task.
        task_sessions.assert_not_called()
    finally:
        await router.stop()
...
async def test_late_duplicate_interactive_reply_never_enters_task_storage(world, redis_outage):
    publisher = AsyncMock()
    publisher.send_command_wait_result.side_effect = HTTPException(504, "isolated timeout")
    with patch("backend.websocket.pubsub_router.get_pubsub_publisher", return_value=publisher):
        response = await world.client.post(f"/api/v1/devices/{world.dev_a.id}/shell",
    ...
    transport = AsyncMock() if redis_outage else world.redis
    if redis_outage:
        transport.publish.side_effect = ConnectionError("isolated Redis outage")
    manager = AsyncMock()
    with (patch("backend.database.redis_client.redis", transport),
          patch("backend.database.engine.AsyncSessionLocal", wraps=world.sessions) as task_sessions):
        for _ in range(2):
            await handle_command_result(str(world.dev_a.id), str(world.org_a.id), {
                "command_id": command['command_id'], "status": "completed", "result": {"output": "isolated"},
            }, manager)
        task_sessions.assert_not_called()
    manager.send_to_device.assert_not_awaited()  # No false durable-result receipt.


async def test_unknown_durable_uuid_still_warns_and_receives_no_ack(world):
    task_id = str(uuid.uuid4())
    manager = AsyncMock()
    with (patch("backend.database.redis_client.redis", world.redis),
          patch("backend.database.engine.AsyncSessionLocal", world.sessions),
          patch("backend.services.task_service.logger.warning") as warning):
        await handle_command_result(str(world.dev_a.id), str(world.org_a.id), {
            "command_id": task_id, "status": "completed", "result": {"success": True},
        }, manager)
    warning.assert_called_once_with("task.result.not_found", task_id=task_id)
