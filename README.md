...
| Связь | [Фоновая регистрация и актуальный device ID](docs/architecture/ANDROID-BACKGROUND-ENROLLMENT.md), отменяемая регистрация, подтверждение авторизации, recoverable refresh, [сохранённый основной/резервный маршрут](docs/architecture/ANDROID-SAVED-ROUTES.md), discovery без credentials | Реальные OS/network/fleet drills, durable config version/rollback и HA backend |
