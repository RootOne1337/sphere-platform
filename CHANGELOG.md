...
- F32-28: добавлен явно включаемый super_admin канал доставки одного APK старым
  ...
  70 регрессий прошли, оба локальных canary обновлены до 1.2.9 через OTA.
  Отдельная приёмка удалённой группы ещё требуется.

- APK 1.2.9: перед командным подключением проверяет привязку виртуальной карты
  и повторно регистрирует скопированную identity; credentials и binding сохраняются
  атомарно. Изменение модели/Android ID не меняет уже выбранную VM-привязку.
  Сборка-кандидат требует backend с поддержкой instance_binding; доставка и
  удалённая приёмка фиксируются в F32-28.
...
- AUD-143 / F32-32: отдельный Redis runtime probe воспроизвёл OOM (`exit 137`,
  ...
  4× baseline. CI probe теперь проходит AOF rewrite, BGSAVE и restart без OOM,
  сохраняя 6,531 ключ; peak cgroup при этом достигает потолка 2048 MiB. Это не
  доказывает spare headroom для streams; pilot rollout ещё не выполнялся.
...
- AUD-107: serialize concurrent OTA attempts, use unique staging files, cancel
  ...
  Both Androids self-install 1.2.4 / 10204 via the server (10.266 / 9.844 s to
  commands). Native truncated response now leaves no file; two acknowledged
  overlapping commands produce one download before replacement (10.453 s).
  Post-OTA reboot recovers in 21.094 s. Twelve final commands and both installed
  hashes pass; signed variants each pass 70 selected tests. LATEST advances to
  fdd26c5 after acceptance; previous 10203 release is retained.
